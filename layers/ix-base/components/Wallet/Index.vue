<template lang="pug">
VList(flex-grow="1" pos="relative" min-h="0" justify="center" p="3" items="center" font="gridnik" text="base")
  WalletConnector()
...
watch(isLoggedInAndConnected, (connected) => {
  if (!process.client || !connected) return;
  ...
  if (redirectQuery) {
    ...
    const ensureRelativeQuery = /^[^\/]+\/[^\/].*$|^\/[^\/].*$/gim;
    if (redirectPath.match(ensureRelativeQuery))
      return navigateTo(redirectPath);
    else return navigateTo(successPath);
  } else {
