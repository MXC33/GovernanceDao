...
PopupFrame(:hideBackButton="true")
  template(#content)
    VList(p="4" pb="8" border-solid="~" border-gray-700="~" z="3" text="md" font="400" color="$mc-whitenew" space-y="8")
      ...
      div()
        Adjustable(v-model="oneTimeLotteryEntries"  w="full" :is-neutral="true" frame="~ hover:gray-300")
        div(mt="1" text="3" color="gray-200") Your entries {{oneTimeLotteryEntries.value}}
...
const onEnterLottery = async () => {
  try {
    console.log(await enterLottery(oneTimeLotteryEntries.value.value))
  } catch (e) {
