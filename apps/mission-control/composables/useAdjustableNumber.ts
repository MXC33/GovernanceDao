import { Ref } from 'vue'
import { CorporationAdjustableToken } from './corporations/useCorporations'
import { clamp } from './useHelpers'
...
export const addAdjustableToToken = (token: NftFragment, options: AdjustableTokenOptions = {}): AdjustableToken => {
  const {
    fromUser = true,
    min = 0,
    max = Infinity,
...
export const addStaticPaymentToken = (token: NftFragment, min: number = 1): CorporationAdjustableToken => {
  return {
    token,
    fromUser: true,
    value: min,
