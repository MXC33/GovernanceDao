...
VList(justify="center" items="center" aspect="4/5" w="50%" bg="black opacity-40")
  ...
  div(aspect="square" w="full")
    div(pos="relative" w="full" h="full")
      TokenVideo(:token="token" v-if="displayVideo" w="full" h="full" :is-large="true" pos="absolute" inset="0" object="cover center" bg="black" :key="getNftTokenKey(token)")
      TokenImage(:token="token" v-else w="full" h="full"  pos="absolute" inset="0" :is-large="true" p="4 lg:8" :key="getNftTokenKey(token) + 'image'")
