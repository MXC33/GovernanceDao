...
VList.mobile-slideout(bg="black opacity-60" backdrop="~ blur-lg" flex-grow="md:1" color="white" pos="sticky left-0 top-auto bottom-0 lt-md:right-0 md:(absolute bottom-0 top-0)" min-w="md:100" w="md:25vw" z="6" b="md:r-1 gray-900" ref="slideoutElement" max-h="full" pointer-events="auto" :style="elementStyle" v-if="isOpen")
  ...
  div(flex="~ col grow" h="full")
    .grabbable(ref="headerElement" @mousedown="touchHandler(true)" @mouseup="touchHandler(false)" @touchstart="touchHandler(true)" @touchend="touchHandler(false)" z="1")
      ...
      GameSettingsSlideoutHeader(@close="$emit('close')" @step-back="$emit('stepBack')" :title="title" :is-nested="isNested" :is-mobile="isMobile") 
        template(#default)
          slot(name="header")
    ...
    VList(overflow-y="auto" overflow-x="hidden" flex-grow="1" scrollbar="~ mc") 
      slot(name="default")
