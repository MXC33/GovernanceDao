var SimpleMoverNew = pc.createScript('simpleMoverNew');

SimpleMoverNew.attributes.add('speed', { type: 'number', default: 0.1 });
SimpleMoverNew.attributes.add('distance', { type: 'number', default: 0.5 });
SimpleMoverNew.attributes.add('duration', { type: 'number', default: 3 });
SimpleMoverNew.attributes.add('waitTime', { type: 'number', default: 3 });

// initialize code called once per entity
SimpleMoverNew.prototype.initialize = function() {
...
SimpleMoverNew.prototype.update = function(dt) {
    if (!this.hasReachedEnd) {
        ...
        if (this.rotateTimer >= this.rotateTime) {
            ...
            //var oppositeDirection = this.entity.forward.clone().rotate(new pc.Quat().setFromEulerAngles(0, 180, 0));
            var oppositeDirection = this.entity.getRotation().transformVector(pc.Vec3.BACK.clone()).scale(-1);
