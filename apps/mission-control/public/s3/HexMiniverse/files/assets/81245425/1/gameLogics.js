...
GameLogics.prototype.updateHighligtOnPix = function(pixCord) {
    ...
    if(pixClicker.isValid(toolStateMode, pixEntity)) {
...
GameLogics.prototype.updateShadowmaps = function() {
    //this.addDebugLine("system  : update_shadows");
...
GameLogics.prototype.setPix = function(x, y, z, tierType, territoryType, pixState, graphicsQuality) {
    ...
    if(currentPix == null || currentPixScript.pixTier != tierType) {
        ...
        //     clone.script.pixScript.setGraphicState(GraphicsQuality.Low);
        // }else{
        //     clone.script.pixScript.setGraphicState(GraphicsQuality.High);
        // }
...
GameLogics.prototype.setToolStateMode = function(toolStateMode) {
    ...
    if(this.app.root == null) { return;}
    /*const isDebugMode = this.app.root.findByName("PlaceholderGUI");
    if(isDebugMode == null) { return; }
    if(isDebugMode.enabled == false){
        if(this.pixClicker){
            return;
        }
        if(this.pixClicker.script){
            return;
        }
    }*/
        ...
        for(var r = -nrOfRings; r<= nrOfRings; r++){
            for(var s = -nrOfRings; s <= nrOfRings; s++){
                ...
                if(isValidHexCord){
                    ...
                    this.setPix(q, r, s, PixType.Empty, TerritoryType.Pix, PixState.Active);
...
//     const index = this.resolutionScalingArray[this.currentMasterGraphicsQuality];
//     this.resolutionScalingEntity.script.resolutionScaling.setScale(index);
//     console.log("graphicsQuality = " + getString(graphicsQuality));
...
//                             pix.pixTier != PixType.MiddleSlot; 
//         if(isNormalPix){
//             pix.setGraphicState(this.currentMasterGraphicsQuality);
//         }
//     });
