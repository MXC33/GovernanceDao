...
Transition(name="fade" mode="out-in" )
  ...
  Table(v-else :columns="columns" :rows="items" :id="context" :loading="loading" :is-open="showFilters" :selectable="true" v-model="selectedTableRows")
...
const selectedTableRows = shallowRef<number[]>([])
...
watch(selectedTableRows, (newItems) => {
  if (!sortedRows.value)
    return

  selectedItems.value = newItems.map((index) => sortedRows.value[index])
})
